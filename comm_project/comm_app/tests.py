...
class UserTestCase(TestCase):
    ...
    def test_user_confirm_verification_code(self):
        ...
        c = Client()
        c.get(reverse("comm_app:confirm_ver_code",
...
class PointsTestCase(TestCase):
    def setUp(self):
        ...
        for i in range(6, 10):
            name = "user" + str(i)
            a = create_user_person(name, name+"@mail.com", name, name,
                                   user.person.invite_code)
            c.post('/login/', {'uname': name, 'pwd': name})
            c.get(reverse("comm_app:generate_code"))
...
class EmailTestCase(TestCase):
    ...
    def test_email_exist(self):
        ...
        self.assertEqual(is_correct_email(
            name+"@mail.com"),
            (False, 'The given email already exists.'))
